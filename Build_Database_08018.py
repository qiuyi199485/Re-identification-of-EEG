    ...
    for p_X in subjects_id:               ##遍历每一个subject
        ...
        if len(sessions) == 1:                                                  
            ...
            for i in range(len(Add_Dataset)):
              Add_Dataset[i] = Add_Dataset[i] + ('B',)    # value '0' means:Subject has 1 Session but more Tokens/Recordings
        # 3. deal with subjects with more than 1 sessions
        else:
             if len(session_dates) == 1:
              S_n_T_1+=1  
              for i in range(len(Add_Dataset)):
                 Add_Dataset[i] = Add_Dataset[i] + ('C',) # value '1' means:Subject has 2 or more Sessions but in same date
             else:  
              S_n_T_n+=1     
              for i in range(len(Add_Dataset)):  
                 Add_Dataset[i] = Add_Dataset[i] + ('D',) # value '2' means:Subject has 2 or more Sessions and in diff dates
