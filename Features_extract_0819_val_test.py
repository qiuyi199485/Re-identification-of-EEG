...
eeg_folder_path = os.path.join(desktop_path, 'EEG_Cleaned_Epochs_val_test')
...
# 根据标签分组，并将每个标签的前10个epoch归入val_set，其余的归入test_set
for label, group in extracted_features_df.groupby('Label'):
    val_set = pd.concat([val_set, group.head(10)], ignore_index=True)
