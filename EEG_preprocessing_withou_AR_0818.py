...
# 将特征和标签整合并导出到Excel
def save_features_to_excel(features, labels, output_path):
    n_files = features.shape[2]
    n_channels, n_features = features.shape[0], features.shape[1]

    # Flatten the features across all epochs for each file
    all_features_flattened = []
    all_labels = []

    for file_idx in range(n_files):
        for epoch_idx in range(features.shape[0]):
            # Flatten the features of the current epoch and append the label
            flattened_feature = features[epoch_idx, :, file_idx].flatten()
            all_features_flattened.append(flattened_feature)
            all_labels.append(labels[file_idx])

    # Create DataFrame
    columns = [f'Feature_{i+1}' for i in range(all_features_flattened[0].shape[0])] + ['Label']
    df_features = pd.DataFrame(all_features_flattened, columns=columns)
...
# 定义输出路径
output_excel_path = os.path.join(desktop_path, "extracted_features_with_labels.xlsx")

# 提取特征并保存
features = extract_features(all_epochs_clean, f_s)
save_features_to_excel(features, label, output_excel_path)

desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
#excel_file_path = os.path.join(desktop_path, 'features.xlsx')
