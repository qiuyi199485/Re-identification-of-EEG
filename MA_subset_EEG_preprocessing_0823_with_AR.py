...
def process_data(df, output_folder):
    ...
    for i in range(0,len(df)):  
        ...
        normalized_epochs = []
        for epoch in epochs_car.get_data():
            normalized_epoch = np.array([normalize_data(epoch[ch]) for ch in range(epoch.shape[0])])
            normalized_epochs.append(normalized_epoch)

        # Convert normalized data back to MNE EpochsArray
        normalized_epochs_array = np.array(normalized_epochs)
        cleaned_epochs = mne.EpochsArray(normalized_epochs_array, info)
